...
type ArtigosCategory = VehicleCategory | CollectibleCategory | 'residencia';

const CATEGORY_ORDER: ArtigosCategory[] = ['carro', 'aviao', 'iate', 'moedas', 'relogios', 'vinhos', 'arte', 'residencia'];

const CATEGORY_META: Record<ArtigosCategory, { label: string; icon: string }> = {
...
function isCollectibleCategory(cat: ArtigosCategory): cat is CollectibleCategory {
  return cat === 'moedas' || cat === 'relogios' || cat === 'vinhos' || cat === 'arte';
}
...
export function ArtigosScreen() {
  const [category, setCategory] = useState<ArtigosCategory>('carro');
  ...
  const vehicleCategory = isVehicleCategory(category) ? category : null;
  const vehiclesInCategoryByPrice = vehicleCategory
    ? state.vehicles.filter((v) => v.category === vehicleCategory).slice().sort((a, b) => a.price - b.price)
    : [];
  const rankById = new Map(vehiclesInCategoryByPrice.map((v, i) => [v.id, i]));
  const vehiclesInCategory = vehicleSort === 'asc' ? vehiclesInCategoryByPrice : vehiclesInCategoryByPrice.slice().reverse();
  const ownedInVehicleCategory = vehiclesInCategory.filter((v) => v.owned).length;

  const collectibleCategory = isCollectibleCategory(category) ? category : null;
  const collectiblesInCategory = collectibleCategory ? state.collectibles.filter((c) => c.category === collectibleCategory) : [];
  ...
  return (
    <div className="screen">
      ...
      <div className="artigos-tile-grid">
        {CATEGORY_ORDER.map((cat) => {
          ...
          if (isVehicleCategory(cat)) {
            ...
            count = `${items.filter((v) => v.owned).length}/${items.length}`;
          } else if (isCollectibleCategory(cat)) {
            const items = state.collectibles.filter((c) => c.category === cat);
            count = `${items.filter((c) => c.owned).length}/${items.length}`;
          }
          return (
            <button
              key={cat}
      ...
      {vehicleCategory && (
        <>
          <div className="section-title-row">
            <h2>{CATEGORY_META[vehicleCategory].label}</h2>
            <span className="counter">{ownedInVehicleCategory}/{vehiclesInCategory.length}</span>
          </div>
      ...
      {collectibleCategory && (
        <>
          <div className="section-title-row">
            <h2>{CATEGORY_META[collectibleCategory].label}</h2>
